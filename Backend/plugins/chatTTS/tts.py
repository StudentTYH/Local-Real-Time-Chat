...
API_BASE_URL_V1 = os.environ.get("ChatTTS_BASE_URL", "http://localhost:9880")
...
class TTS(tts.TTS):
    def __init__(
        ...
        Args:
            model (TTSModels | str): TTS model to use. Defaults to "male.wav".    tts模型
            base_url (str | None): Custom base URL for the API. Optional.      api接口
        ...
        self._base_url=base_url or API_BASE_URL_V1
    ...
    def update_speaker(
        ...
        Args:
            ...
            model (TTSModels | str): TTS model to use. Defaults to "eleven_turbo_v2_5".
        """
        self._opts.tts_speaker = speaker or self._opts.tts_speaker
...
class ChunkedStream(tts.ChunkedStream):
    ...
    def __init__(
        ...
        super().__init__(tts=tts, input_text=text, conn_options=opts)
    ...
    async def _main_task(self) -> None:
        ...
        payload = {
            ...
            "media_type": "wav",
            "seed": self._opts.tts_speaker,
            "streaming": 1,
        }

        try:
            async with self._session.post(
                self._opts.base_url,json=payload,
                headers={AUTHORIZATION_HEADER: self._opts.api_key},
            ) as resp:
...
def _synthesize_url(opts: _TTSOptions, text: str) -> str:
    import urllib.parse
    default_speaker =opts.tts_speaker
    default_language = os.environ.get("XTTS_LANGUAGE", "zh-CN")
